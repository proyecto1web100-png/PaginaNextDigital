/* eslint-disable @typescript-eslint/ban-ts-comment, @typescript-eslint/no-unused-vars, @typescript-eslint/no-unused-expressions -- vendored from VengeanceUI registry */
'use client';
                    ...
                    {items.map((el, index) => {
                        ...
                        const handleClick = () => {
                            if (el.onClick) {
                                ...
                                // External links (social networks) open in a new tab.
                                if (/^https?:/.test(el.href)) window.open(el.href, '_blank', 'noopener,noreferrer');
                        ...
                        return (
                            <div
                                ...
                                onClick={handleClick}
                                className="relative w-10 h-10 flex items-center justify-center cursor-pointer rounded-xl outline-none focus-visible:ring-2 focus-visible:ring-brand"
                                role={el.href && /^https?:/.test(el.href) ? 'link' : 'button'}
                                aria-label={el.title}
                                tabIndex={0}
                                onFocus={() => handleMouseEnter(index)}
                                onBlur={() => { setHoveredIndex(null); setDirection(0); }}
                                onKeyDown={(e) => {
